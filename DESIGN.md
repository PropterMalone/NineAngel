...
16. **Freshness** (`requires: package_json | deps_lockfile | ci_config`; opt-in since ADR-07 demotion 2026-07-08) — *Is this still true?* Finds: stale deps, hardcoded URLs/dates, outdated config, assumptions about external APIs that may have changed, deprecated patterns. Demoted from the default battery: 4% acceptance rate, 0 ground-truth catches in eval legs 1–3.
...
21 personas is a lot — the default move is **consolidate, not add**. (PII-Sweep + De-Anon were added 2026-06-04 as a paired privacy lane for a recurring need; Editor + Rigor added 2026-07 as a prose-quality lane gated on `prose_artifacts`; Heir added 2026-07 as a cold-start handoff gate; Recipient added 2026-07-28 as the output-usefulness lane; all ship experimental or opt-in, so they cost nothing on runs that don't name them.) Every persona costs tokens on every run it fires in, and overlapping personas dilute the signal without adding coverage. Resist adding new personas until the data can adjudicate which existing ones earn their slot.

**Why Recipient was an add and not a lane-widening** (the alternative considered and rejected): widening `user`'s lane to include rendered outputs would have collapsed two different failure modes into one persona. `user` reviews *interaction* — flows, error text, feedback, empty states; Recipient reviews *value of what was produced*. A pipeline can score perfectly on the first and emit a useless artifact. The 2026-07-22 redundancy analysis localized real redundancy to the *broad* personas (hyper/future/thousand/rtfm/naive/adv); Recipient is narrow by construction and gated on an artifact, so it never enters a default battery. **Could-be-wrong-if:** across its first ~5 live runs, ≥60% of Recipient's Important+ findings are co-attributed to `user` or `editor` — that would mean the lane is a re-slice of existing coverage, not a gap, and the right move is to fold its five tests into `user` and retire it.
...
**Calibration — concluded (2026-06-01), reader not promoted**: a 13-project live-use calibration (auto-triggered baseline+reader double-runs, paired snapshots, `reader-calibration.json` markers) adjudicated the reader against a three-axis gate (cost/speed/quality). It failed decisively — avg +17.4% tokens / +49% wall, never cheaper at any size 2.4K–60K LOC, quality a noisy wash. The reader stays opt-in permanently; the §1.6 auto-trigger machinery was removed from SKILL.md 2026-06-09. Full evidence and the reopen condition (a re-implemented slicer with median bundle <~30% of full-project size) in `docs/decisions/01-reader-default-off.md`.
