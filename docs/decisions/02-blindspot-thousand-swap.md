...
**How to apply**: Binds default-battery selection on `/angel --full` runs, and — a consequence missed at decision time — on diff runs too: demoting Thousand-Foot removed the only diff-default top-tier absence/architecture reasoner, so plain diff runs now carry zero of that lane. **Correction 2026-06-12** (full meta-run finding f3): the original "diff runs are unaffected" claim was false. The diff-mode gap is consciously accepted until the review trigger (the user, 2026-06-12); interim opt-in is `/angel thousand` on diff runs that want the lane. At the review trigger (≥~12 Blindspot full-runs), decide (a) Blindspot keep vs revert, (b) Thousand-Foot scrap vs re-tune. If Thousand-Foot is kept, re-tune its lane *up* to strategic direction ("is the overall approach right" — queue-not-cron, whole-module duplication) so it complements rather than duplicates Blindspot. Do not return either persona to the default stack without precision data.
